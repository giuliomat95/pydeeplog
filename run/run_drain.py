...
line_count = 0
with open(os.path.dirname(os.path.dirname(os.path.realpath(__file__))) + input_dir + args.input) as f:
...
with open(args.output + '/data.json', 'w') as f:
    json.dump(result, f)
    f.close()
with open(args.output + '/templates.json', 'w') as g:
    json.dump(templates, g)
    g.close()
#let's save the Drain tree object in a pickle file
with open(args.output + '/drain_tree.pickle', 'wb') as h:
