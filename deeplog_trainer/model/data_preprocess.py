...
class DataPreprocess:
    ...
    def chunks_seq(self, seq):
        """
        Splits a sequence in smaller chunks with window_size as maximum length. Return a list of lists of size
        (len(seq)-window_size+1,window_size)
        """
        chunks = []
        if len(seq) > self.window_size:
            # If the sequence is longer than the window size, drag the window and split as many sequences as possible
            i = 0
    ...
    def transform(self, dataset, add_padding=0):
        ...
        if add_padding > 0:
            X_data = pad_sequences(X_data, maxlen=add_padding, value=self.dict_token2idx['[PAD]'], padding='post')
            # Pads input sequences. The ones whose length is smaller than 'maxlen' padded with 'value' until they reach
            # all the same length
        # One hot encoding: Return vectors with all zeros but 1 in the in the indices position passed in input.
